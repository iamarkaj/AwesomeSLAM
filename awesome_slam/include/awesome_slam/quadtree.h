/*
src: https://github.com/spauka/SLAM/blob/master/occupancygrid/quadtree.h
*/
...
struct Cell
{
    ...
    Cell(int x, int y, int width, char value) : x(x), y(y), index(x + y * width), value(value) {}
...
struct Quadtree
{
    ...
    Quadtree(int x, int y, int size, Cell &cell) : x(x), y(y), size(size), cell(cell), isLeaf(true)
    {
...
struct Map
{
    ...
    Map(int width, int height) : cells(NULL), width(width), height(height), cell_count(0) {}
